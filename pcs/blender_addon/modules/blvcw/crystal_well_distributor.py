...
class VCWSimpleDistributor(CrystalWellDistributor):
    ...
    def _minimize(self, crystal, target_area):
        # scale estimation
        width, height, _ = self._get_min_area_rect(crystal)
        area = width * height / self.image_area
        ...
        while oob:
            cam_coords = self._to_camera_coords(crystal)
            if self._in_bounds(crystal, cam_coords):
                oob = False
            else:
                scale0 *= 0.9
    ...
    def _place_crystal(self, crystal):
        ...
        while n_tries < 10000:
            ...
            res = self._minimize(crystal, target_area)
